 ...
 int Attack(lua_State *L)
{
	...
	int i;
	for( i = 0; i < 10; i ++ ){
		if( pEntry[iNum[i]].char_index != -1 ){
			break;
		}
	}

	CHAR_setWorkInt( char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK );
	CHAR_setWorkInt( char_index, CHAR_WORKBATTLECOM2, iNum[i] + side * BATTLE_ENTRY_MAX);
	int At_SoubiIndex =  CHAR_getItemIndex( char_index, CHAR_ARM );
	if( ITEM_CHECKINDEX( At_SoubiIndex ) == TRUE )
	{
		if( ITEM_getInt( At_SoubiIndex, ITEM_TYPE ) == ITEM_BOOMERANG ){
			...
			if( CHAR_getInt( char_index, CHAR_BASEIMAGENUMBER)==101749
				|| CHAR_getWorkInt( char_index, CHAR_WORKFOXROUND ) != -1 ){ //若是变成小狐狸不可以使用回力标
 ...
 int Recovery(lua_State *L)
{
	...
	CHAR_setWorkInt( char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_OFFLINE_RECOVERY );
