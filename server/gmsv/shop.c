...
void ShopData_Init() { memset(&gShopData, 0, sizeof(ShopData) * 5); }
...
void WriteShopData(char *data, int id) {
  ...
  if (*data != 0) {
    ...
    int i;
    for (i = 0; i < 16; i++) {
      snprintf(gShopData[id].shopMd5 + i * 2,
               sizeof(gShopData[id].shopMd5 + i * 2), "%02X",
               (unsigned char)szDigest[i]);
    }
    ...
    if (*gShopData[i].shopMd5) {
      if (strcmp(gShopData[i].shopMd5, shop[i]) || !shop[i]) {
        if (gShopData[i].shopData) {
          GmsvServer_UpShopData_send(fd, gShopData[i].shopData,
                                     gShopData[i].shopMd5, i);
        }
      }
    }
  }
